...
def monitor() = Actor[Event, Unit] {
  ...
  receive { (event: Event, self: ActorRef[Event]) => event match
    ...
    case (MaintenanceRequest(mid1, rid1, ts1),
          RequestCompleted(_, rid2, _),
          MaintenanceRequest(mid3, rid3, ts3)) if rid1 == rid2
                                                  && mid1 == mid3
    ...
    case (MaintenanceRequest(mid1, rid1, ts1),
          RequestTaken(_, rid2, ts2)) if rid1 == rid2
                                         && ts2 - ts1 >= TEN_MIN =>
      ...
      Next()
    case (MaintenanceRequest(mid1, rid1, ts1),
          Mark(ts2, used)) if ts2 - ts1 >= TEN_MIN
                              && !used.contains(rid1) =>
      self ! MaintenanceRequest(mid1, rid1, ts1) // Re-enqueue latest request
    ...
    case (Mark(ts1, _),
          Mark(ts2, used2)) if ts2 - ts1 >= 2 * TEN_MIN =>
    ...
    case (MaintenanceRequest(mid1, _, ts1),
          MaintenanceRequest(mid2, _, ts2),
          MaintenanceRequest(mid3, _, ts3)) if mid1 == mid2 && mid2 == mid3
                                               && ts1 <= ts2 && ts2 <= ts3
                                               && ts3 - ts1 < ONE_DAY =>
