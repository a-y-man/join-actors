...
enum SystemEvent:
  case DelayedMaintenanceRequest(machineId: Int, reqId: Int, reason: String,
                                 ts: Long)
  // ...
  case Shutdown()
...
def monitor() = Actor[Event, Unit] {
  ...
  receive { (event: Event, self: ActorRef[Event]) => event match
    case (MaintenanceRequest(_, rid1, _, ts1),
          RequestTaken(_, rid2, ts2)) if rid1 == rid2 =>
      // updateMaintenanceStats(ts1, ts2)
      Next()

    case (MaintenanceRequest(mid, rid1, reason, ts1),
          MaintenanceRequest(_, rid2, _, ts2),
          RequestTaken(wid, rid3, ts3)) if rid2 == rid3
                                           && ts2 > ts1 + TEN_MIN =>
    ...
    case (DelayedMaintenanceRequest(_, rid1, _, ts1),
          RequestTaken(_, rid2, ts2)) if rid1 == rid2 =>
      // updateMaintenanceStats(ts1, ts2)
      Next()
