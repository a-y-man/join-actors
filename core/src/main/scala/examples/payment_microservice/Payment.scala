...
import join_actors.examples.payment_microservice.*
...
class Payment(private val algorithm: MatcherFactory):
  def prepare(param: Int): PaymentPrereqs =
    ...
    val (res, coreService: PaymentActor) =
      getCoreServiceActor(accountServiceCell, tokenServiceCell, paymentServiceCell, param).start()
    val (_, accountService: PaymentActor) =
      getAccountServiceActor(paymentServiceCell, tokenServiceCell).start()
    val (_, tokenService: PaymentActor) =
      getTokenServiceActor(accountServiceCell, coreServiceCell).start()
    ...
    val matcher = receive { selfRef =>
      {
        case PaymentRequested(id1)
            ...
            &:& CustomerValidated(id3) if (id1 == id2) && (id2 == id3) =>
  ...
  private def getAccountServiceActor(
  ...
  private def getTokenServiceActor(
      accountService: RefCell[PaymentActor],
      coreService: RefCell[PaymentActor]
  ) =
    val matcher = receive { selfRef =>
      {
        case PaymentRequested(id) =>
        ...
        case TokenGenerationRequested(id1)
            &:& CustomerValidated(id2) if id1 == id2 =>
