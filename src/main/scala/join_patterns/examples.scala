...
object PingPong extends App:
  type Ponger = ActorRef[Ping]
  type Pinger = ActorRef[Pong]
  ...
  def pingActor() =
    val actor: Actor[PingPong, Unit] =
      Actor[PingPong, Unit](receive { (y: PingPong, pingRef: Pinger) =>
        y match
          case Pong(pongRef, x) =>
            if x < 100 then
              val x = 42
  ...
  def pongActor() =
    val actor: Actor[PingPong, Unit] =
      Actor[PingPong, Unit](receive { (x: PingPong, pongRef: Ponger) =>
        x match
          case Ping(pingRef, x) =>
            if x < 100 then
              val x = 42
